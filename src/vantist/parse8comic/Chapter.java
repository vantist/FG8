...
public class Chapter {
	...
	public Chapter(VUtil vUtil, String url) {
	...
	private String getPicContent() {
		return vUtil.getUrlContent(url);
	}
	...
	private int getChs(String content) {
		...
		return Integer.parseInt(vUtil.getRegexList(content, pattern).get(0).split("[=;]")[1]);
	}
	
	private String parseComicName(String content) {
		String pattern = ":\\[(.)*<font id=";
		
		return vUtil.getRegexList(content, pattern).get(0).split(" ")[1];
	}
